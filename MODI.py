...
Some problems:
1. Although I assigned epsilon to be 1.e-9, there might still be some small calculation errors when finding the total cost. The ideal
case is to leave all the epsilons and multiply the solution with the cell's cost and then add all of them.
2. Need to rewrite the code to be a non-recursive one. Both in search_row & search_col and in check_solution & find_loop (better use
loops instead).
3. When considering degeneracy, there might be the case when the cost cells have the same value, so even if I delete the first value
in the sort list, I'm not sure if I didn't repeatedly check the cells.
4. The inputs of the code can be rewritten so that it can just read from a file.
'''
